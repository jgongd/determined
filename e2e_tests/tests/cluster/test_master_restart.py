...
def _test_exp_restart_allocations_start_time(managed_cluster_restarts: Cluster) -> None:
    ...
    for rp in resp.get("results"):
        for period in rp.get("aggregates"):
            print(f"duration: {period['seconds']}")
            assert period['seconds'] <= EXPECTED_MAX_AVG_ALLOCATION_QUEUED_SECONDS


# @pytest.mark.e2e_cpu
# def test_exp_restart_allocations_start_time_tmp() -> None:
#     exp_id = exp.create_experiment(
#         conf.fixtures_path("no_op/single-medium-train-step.yaml"),
#         conf.fixtures_path("no_op"),
#         ["--config", "hyperparameters.metrics_sigma=-1.0"],
#     )
#     exp.wait_for_experiment_state(exp_id, EXP_STATE.STARTING)

#     command = ["det", "-m", conf.make_master_url(), "task", "list", "--json"]
#     tasks_data = json.loads(subprocess.check_output(command).decode())
#     assert len(tasks_data) == 1
#     allocation_id1 = [allocation_id for allocation_id, _ in tasks_data.items()][0]
#     print(allocation_id1)


#     exp.wait_for_experiment_state(exp_id, EXP_STATE.ERROR)
...
#     tasks_data = json.loads(subprocess.check_output(command).decode())
#     assert len(tasks_data) == 1
#     allocation_id2 = [allocation_id for allocation_id, _ in tasks_data.items()][0]
#     print(allocation_id2)
